...
class BasicBlock(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, norm=False):
        ...
        body = [
           ...
           # nn.Conv2d(in_channels, out_channels, kernel_size=3,stride=stride, padding=1, groups=out_channels,
           #                             bias=False),
            nn.LeakyReLU(0.2)
        ]
        if norm:
...
class LightBackbone(nn.Sequential):
    ...
    def __init__(self,
        ...
        self.out_channels = n_feats * (
            4 if extra_pooling else (self.input_resolution // 32) ** 2)
        print(self.out_channels)

    def forward(self,imgs):
        imgs = F.interpolate(imgs, size=(self.input_resolution,) * 2,
            mode='bilinear', align_corners=False)
        return super().forward(imgs).view(imgs.shape[0], -1)  #Reshape 3 × N × N × N
